...
public function editClientAction(Request $request,$id){
    ...
    if( $this->get('security.context')->getToken()->getUser()->hasRole("ROLE_SUPER_ADMIN") ||
        $client->getUser()->getId() == $this->get('security.context')->getToken()->getUser()->getId()
        )
    {
...
public function editCompanyAction(Request $request,$id){
    ...
    if( $this->get('security.context')->getToken()->getUser()->hasRole("ROLE_SUPER_ADMIN") ||
        $company->getUser()->getId() == $this->get('security.context')->getToken()->getUser()->getId()
        ){
    $form = $this->get('form.factory')->createBuilder('form',$company)
    ->add("Nom","text")
    ->add("adresse","text")
...
public function editOportunityAction(Request $request,$id){
    ...
    $client = $em->getRepository('BGPlateformBundle:Client')->getClientFromOportunity($id);
     if( $this->get('security.context')->getToken()->getUser()->hasRole("ROLE_SUPER_ADMIN") ||
        $client->getUser()->getId() == $this->get('security.context')->getToken()->getUser()->getId()
        ){
    $form = $this->get('form.factory')->createBuilder('form',$oportunity)
